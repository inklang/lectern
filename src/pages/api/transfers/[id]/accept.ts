...
import { getTransferRequest, acceptTransfer } from '../../../../lib/db.js'
...
export const POST: APIRoute = async ({ params, request }) => {
  ...
  if (transfer.to_owner_id !== userId) {
    ...
    if (transfer.to_owner_type === 'org') {
      ...
      if (!admin) {
        return new Response(JSON.stringify({ error: 'Only the recipient or org admin can accept this transfer' }), { status: 403 })
      }
    } else {
      return new Response(JSON.stringify({ error: 'Only the recipient can accept this transfer' }), { status: 403 })
    }
  }
  ...
  if (new Date(transfer.expires_at) < new Date()) {
    return new Response(JSON.stringify({ error: 'Transfer has expired' }), { status: 409 })
  }
  ...
  try {
    ...
    // Notify initiator
    await emitNotification(transfer.from_owner_id, 'transfer_accepted', {
      transfer_id: id,
      package: result.newSlug,
